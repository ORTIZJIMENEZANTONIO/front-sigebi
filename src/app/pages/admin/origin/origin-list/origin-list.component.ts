...
export class OriginListComponent extends BasePage {
  ...
  public editRow(event) {
    ...
    const modalRef = this.windowService.open(OriginDetailComponent, {
      title: `Editar`,
      context: {
        data: event.data
      },
  ...
  public openWindow() {
    ...
    const modalRef = this.windowService.open(OriginDetailComponent, { title: `Nuevo`, buttons: buttonsConfig }).onClose.subscribe(() => {
      this.read(this.pageEvent.pageIndex = 0, this.pageEvent.pageSize);
    });
