...
export class TypeSinisterListComponent extends BasePage {
  ...
  constructor(
    private service: SiniesterService,
  ...
  public editRow(event) {
    ...
    const modalRef = this.windowService.open(TypeSinisterDetailComponent, { title: `Editar`, context: { data: event.data }, buttons: buttonsConfig }).onClose.subscribe(() => {
  ...
  public openWindow() {
    ...
    const modalRef = this.windowService.open(TypeSinisterDetailComponent, { title: `Nuevo`, buttons: buttonsConfig }).onClose.subscribe(() => {
      this.read(this.pageEvent.pageIndex = 0, this.pageEvent.pageSize);
    });
