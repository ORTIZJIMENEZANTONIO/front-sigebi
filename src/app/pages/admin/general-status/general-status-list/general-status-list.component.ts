...
import { GeneralStatusInterface } from '../../../../@core/interfaces/auction/general-status.model';
...
export class GeneralStatusListComponent extends BasePage {
  ...
  constructor(
    ...
    this.searchForm.controls['text'].valueChanges.subscribe((value: string) => {
      if (value.length > 0) {
        // this.service.search(value).subscribe((rows: GeneralStatusInterface[]) => {
  ...
  public editRow(event) {
    ...
    const modalRef = this.windowService.open(GeneralStatusDetailComponent, { title: `Editar`, context: { data: event.data }, buttons: buttonsConfig }).onClose.subscribe(() => {
  ...
  public openWindow() {
    ...
    const modalRef = this.windowService.open(GeneralStatusDetailComponent, { title: `Nuevo`, buttons: buttonsConfig }).onClose.subscribe(() => {
      this.read(this.pageEvent.pageIndex = 0, this.pageEvent.pageSize);
    });
