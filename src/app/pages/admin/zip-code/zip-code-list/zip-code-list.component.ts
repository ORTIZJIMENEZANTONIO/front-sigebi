...
import { ZipCodeInterface } from '../../../../@core/interfaces/auction/zip.-code.model';
...
export class ZipCodeListComponent extends BasePage {
  ...
  constructor(
    ...
    this.searchForm.controls['text'].valueChanges.subscribe((value: string) => {
      if (value.length > 0) {
        // this.service.search(value).subscribe((rows: ZipCodeInterface[]) => {
  ...
  public editRow(event) {
    ...
    const modalRef = this.windowService.open(ZipCodeDetailComponent, { title: `Editar`, context: { data: event.data }, buttons: buttonsConfig }).onClose.subscribe(() => {
  ...
  public openWindow() {
    ...
    const modalRef = this.windowService.open(ZipCodeDetailComponent, { title: `Nuevo`, buttons: buttonsConfig }).onClose.subscribe(() => {
      this.read(this.pageEvent.pageIndex = 0, this.pageEvent.pageSize);
    });
