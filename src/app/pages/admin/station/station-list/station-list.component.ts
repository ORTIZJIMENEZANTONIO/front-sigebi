...
export class StationListComponent extends BasePage {
  ...
  public editRow(event) {
    ...
    const modalRef = this.windowService.open(StationDetailComponent, { title: `Editar`, context: { data: event.data }, buttons: buttonsConfig }).onClose.subscribe(() => {
  ...
  public openWindow() {
    ...
    const modalRef = this.windowService.open(StationDetailComponent, { title: `Nuevo`, buttons: buttonsConfig }).onClose.subscribe(() => {
      this.read(this.pageEvent.pageIndex = 0, this.pageEvent.pageSize);
    });
