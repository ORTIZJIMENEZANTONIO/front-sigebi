import { Injectable } from '@angular/core';
import { Observable, of } from 'rxjs';
import { DataSource } from 'ng2-smart-table/lib/lib/data-source/data-source';
import { CatalogApi } from '../api/catalog-api';
import { Doccompensation } from '../../../interfaces/auction/doccompensation.model';
import { ClaimsStatus } from '../../../interfaces/auction/claims-status.model';

@Injectable()
export class ClaimsStatusService {
    constructor(private api: CatalogApi) { }

    get gridDataSource(): DataSource {
        return this.api.dataSource;
    }

    url = "claims-status";
    
    list(pageNumber: number = 1, pageSize: number = 10) {
        const data = this.api.list(pageNumber, pageSize,this.url);
        console.log(data)
        return data;
    }
    register(legendData: ClaimsStatus): Observable<ClaimsStatus>{
        return this.api.register(legendData, this.url);
    }

    update(id:number, legendData: ClaimsStatus): Observable<ClaimsStatus>{
        return this.api.update(id, legendData, this.url);
    }

    delete(id:number){
        return this.api.delete(id, this.url);
    }
    search(text:string){
        return this.api.search(text,this.url);
    }
}